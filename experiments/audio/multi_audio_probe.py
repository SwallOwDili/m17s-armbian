#!/usr/bin/env python3
"""Probe hardware HEVC video plus live switching of an external multi-track audio file."""
import argparse
import json
...
try:
    ...
    audio = Gst.ElementFactory.make('playbin3', 'audio-playbin3')
    if audio is None:
        raise RuntimeError('could not create playbin3')
    ...
    def audio_output_probe(_pad, info):
        if info.type & Gst.PadProbeType.EVENT_DOWNSTREAM:
            ...
            if event is not None and event.type == Gst.EventType.STREAM_START:
                result['audio_sink_input']['stream_id'] = event.parse_stream_start()
    ...
    audio_bus = audio.get_bus()
    audio_streams = []
    selected_ids = []
    selected_ever = set()
    while True:
        message = audio_bus.pop_filtered(Gst.MessageType.STREAM_COLLECTION |
                                         Gst.MessageType.STREAMS_SELECTED |
                                         Gst.MessageType.ERROR | Gst.MessageType.WARNING |
                                         Gst.MessageType.QOS)
        if message is None:
            break
        if message.type == Gst.MessageType.STREAM_COLLECTION:
            audio_streams = audio_stream_records(message.parse_stream_collection())
            result['audio_streams'] = audio_streams
            emit({'audio_stream_collection': audio_streams})
        elif message.type == Gst.MessageType.STREAMS_SELECTED:
            selected_ids = [message.streams_selected_get_stream(index).get_stream_id()
                            for index in range(message.streams_selected_get_size())]
            selected_ever.update(selected_ids)
            result['selections'].append({'elapsed_seconds': 0.0, 'phase': 'preroll',
                                         'selected_ids': selected_ids})
        elif message.type == Gst.MessageType.QOS:
            result['qos']['count'] += 1
            result['qos']['last'] = {'source': message.src.get_name() if message.src else None,
                                     'timestamp': int(message.timestamp),
                                     'seqnum': message.get_seqnum()}
        else:
            message_record(message)
    if result['errors']:
        raise RuntimeError('audio preroll posted an error')
    n_audio = len(audio_streams)
    if n_audio <= 0:
        raise RuntimeError('playbin3 did not publish an audio stream collection during preroll')
    ...
    initial_audio_streams = list(audio_streams)
    target_ids = [initial_audio_streams[track]['id'] for _, track in schedule]
    required_target_ids = set(target_ids)
    current_requested = schedule[0][1]

    def select_track(track, elapsed, allow_preroll_match=False):
        stream_id = initial_audio_streams[track]['id']
        accepted = audio.send_event(Gst.Event.new_select_streams([stream_id]))
        record = {'elapsed_seconds': round(elapsed, 3), 'requested_index': track,
                  'requested_id': stream_id, 'event_accepted': accepted,
                  'selection_matched': allow_preroll_match and stream_id in selected_ever}
        result['switches'].append(record)
        emit({'audio_switch_requested': record})
        if not accepted:
            raise RuntimeError(f'playbin3 rejected SELECT_STREAMS for audio index {track}')
        ...
        for label, bus in buses:
            while True:
                message = bus.pop_filtered(Gst.MessageType.EOS | Gst.MessageType.ERROR |
                                           Gst.MessageType.WARNING | Gst.MessageType.STREAM_COLLECTION |
                                           Gst.MessageType.STREAMS_SELECTED | Gst.MessageType.QOS)
                if message is None:
                    break
                ...
                elif label == 'audio' and message.type == Gst.MessageType.STREAM_COLLECTION:
                    audio_streams = audio_stream_records(message.parse_stream_collection())
                    result['audio_streams'] = audio_streams
                    emit({'audio_stream_collection': audio_streams})
                    if current_requested >= len(audio_streams):
                        raise RuntimeError(f'requested audio index outside updated collection: {current_requested}')
                    select_track(current_requested, elapsed)
                elif label == 'audio' and message.type == Gst.MessageType.STREAMS_SELECTED:
                    message.parse_streams_selected()
                    selected_ids = [message.streams_selected_get_stream(index).get_stream_id()
                                    for index in range(message.streams_selected_get_size())]
                    selected_ever.update(selected_ids)
                    for switch in result['switches']:
                        if not switch['selection_matched'] and switch['requested_id'] in selected_ids:
                            switch['selection_matched'] = True
                            switch['matched_elapsed_seconds'] = round(elapsed, 3)
                            break
                    selected_audio = [entry for entry in audio_streams if entry['id'] in selected_ids]
                    record = {'elapsed_seconds': round(elapsed, 3), 'selected_ids': selected_ids,
                              'selected_audio': selected_audio}
                    result['selections'].append(record)
                    emit({'audio_streams_selected': record})
                elif message.type == Gst.MessageType.QOS:
                    result['qos']['count'] += 1
                    result['qos']['last'] = {
                        'pipeline': label,
    ...
    result.update(elapsed_seconds=round(elapsed, 3), decoded_frames=frames[0],
                  rendered_frames=fps.get_property('frames-rendered'),
                  dropped_frames=fps.get_property('frames-dropped'),
                  n_audio=len(audio_streams), requested_audio=current_requested,
                  selected_ids=selected_ids,
                  ...
                  required_audio_ids=sorted(required_target_ids),
                  selected_audio_ids=sorted(selected_ever),
                  all_target_ids_selected=required_target_ids.issubset(selected_ever),
...
raise SystemExit(0 if (not result['errors'] and result.get('both_eos', False) and
                       ...
                       result.get('all_target_ids_selected', False)) else 1)
